...
import { 
  ...
  List, X, Settings, Sun, Moon, Coffee, Bot, Maximize, Minimize, Type
} from 'lucide-react'
...
export default function PostView() {
  ...
  // UX States
  const [readMode, setReadMode] = useState('dark') // 'light', 'dark', 'sepia', 'chatgpt'
  ...
  const toggleFocusMode = async () => {
    try {
      ...
      // Fallback to just UI focus mode if Fullscreen API fails/is blocked
  ...
  // Centralized Theme Configuration for ALL UI Elements
  const themes = {
    light: {
    ...
    <div className={`relative min-h-screen overflow-x-hidden transition-colors duration-500 ${activeTheme.app} ${fontFamily} text-base md:text-lg`}>
      ...
      <div className={`transition-all duration-500 ease-in-out ${isSidebarOpen && !isFocusMode ? 'md:pr-72' : ''}`}>
        ...
        <div className={`mx-auto px-4 py-12 transition-all duration-700 ease-in-out ${isFocusMode ? 'max-w-5xl py-16 md:py-24' : 'max-w-3xl'}`}>
          ...
          {/* Title repeats cleanly if in focus mode without meta info */}
          {isFocusMode && (
      ...
      {/* Slidable Right Sidebar (Table of Contents) - Hidden in Focus Mode */}
      <div 
        className={`fixed right-0 top-0 h-full w-72 border-l shadow-2xl z-50 transform transition-transform duration-300 ease-in-out ${activeTheme.panel} ${isSidebarOpen && !isFocusMode ? 'translate-x-0' : 'translate-x-full'}`}
      >
        <div className="p-5 flex items-center justify-between border-b border-current/10">
          ...
          <button onClick={() => setIsSidebarOpen(false)} className={`rounded-full p-2 transition-colors ${activeTheme.btnInactive}`}>
            <X size={18} />
          </button>
        </div>
      ...
      <div className={`fixed bottom-6 z-40 flex flex-col gap-3 transition-all duration-300 ease-in-out ${isSidebarOpen && !isFocusMode ? 'right-6 md:right-[19.5rem]' : 'right-6'}`}>
        ...
        {/* Toggle Table of Contents (Disabled visual if in focus mode, since sidebar is hidden) */}
        {!isFocusMode && (
          <button 
